...
const MusicFileListItem = SortableElement((props) => {
  const [showDetails, changeShowDetails] = React.useState(true);
  return (
    <div>
      <div
        style={
        ...
        <li className="music-file-list-item">
          ...
          <img
            onClick={() => {
              ...
              var next_song =
                props.musicFiles[
                  props.musicFiles.findIndex(
                    (el) => el.filename === props.file.filename
                  ) + 1
                ];
              if (next_song) {
                props.onSetCurrentSong(next_song);
              }
            }}
            alt="remove song"
            src={DeleteIcon}
